...
from networks.layers import DoubleConv, OutConv, Down, Up, Self_Attn_Layer, Cross_Attn_Layer
...
class UNet_Attention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #### self-attention & cross attention add here

        x_self_1 = self.self_attn1(x5)
        x_self_2 = self.self_attn1(x_self_1)
        x_self_3 = self.self_attn1(x_self_2)
        x_self_4 = self.self_attn1(x_self_3)
        # x_self_5 = self.self_attn1(x_self_4)

        x_cross_1 = self.cross_attn1(x_self_3)
        x_cross_2 = self.cross_attn1(x_cross_1)
        x_cross_3 = self.cross_attn1(x_cross_2)
        x_cross_4 = self.cross_attn1(x_cross_3)
        # x_cross_5 = self.cross_attn1(x_cross_4)
        # x_self_1 = self.self_attn1(x5)

        # x_cross_1 = self.cross_attn1(x_self_1)

        #### UP
        x4_up_pts = self.up1_pts(x_cross_4, x4)
        ...
        f5 = F.interpolate(x_cross_4, size=(height, width), mode='bilinear')
