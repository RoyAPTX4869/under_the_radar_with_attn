...
class UnderTheRadar(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        # data_size = data.size()   # （Batch_size*Window_size，1，Height，Width）
        # detector_scores, weight_scores, desc = self.unet(data)
        detector_scores, weight_scores, desc = self.unet_attn(data)

        # detector_scores_size = detector_scores.size()     #   (Batch_size*Window_size,  1,  Height，Width)
        # weight_scores_size = weight_scores.size()         #   (Batch_size*Window_size,  Score,  Height，Width), S = 1
        # desc_size = desc.size()                           #   (Batch_size*Window_size,  Channel,  Height，Width),     Channel = 256

        keypoint_coords, keypoint_scores, keypoint_desc = self.keypoint(detector_scores, weight_scores, desc)

        # keypoint_coords_size = keypoint_coords.size()     #   (Batch_size*Window_size,  Num_patches,  2)
        # keypoint_scores_size = keypoint_scores.size()     #   (Batch_size*Window_size,  Score,  Num_patches)
        # keypoint_desc_size = keypoint_desc.size()         #   (Batch_size*Window_size,  Channel,  Num_patches)
